...
void build_domain(subset D,vector<value> value_domain[15])
{
	...
	for(int i=0;i<D.attr_list.size();i++)
	{
		...
		for(int j=0;j<D.ent_list.size();j++)
		{
			...
			if(visited[val]==-1)//判断这个取值是否在之前出现过 
			{
				value v;
				v.val=val;
				v.sum=1;
				//统计这个取值的对象是哪一类的 
				if(train_tag[D.ent_list[j]]==0)
				{
					v.cnt[0]=1;
					v.cnt[1]=0;
				}	
	...
	{	
		...
		else if(train_tag[D.ent_list[j]]==0)
...
vector<subset> divide_data(subset D,int a,vector<value> value_domain[15])
{
	...
	for(int i=0;i<value_domain[a].size();i++)
	{
		...
		{
			if(k!=a)
			sub.attr_list.push_back(D.attr_list[k]);//构造属性LIST 
		}
		for(int k=0;k<D.ent_list.size();k++)
		{
			if(train_attr[D.ent_list[k]][D.attr_list[a]]==value_domain[a][i].val)//找到与当前考察属性的值相同的对象 
			sub.ent_list.push_back(D.ent_list[k]);//构造数据集LIST 
		}
	...
	for(int k=0;k<ent;k++)
	{
		...
		while(child!=NULL)
		{
			...
			for(int i=0;i<n;i++)
			{
				...
				{
					if(child[i]->result==1||child[i]->result==0)
